			  ...
			  <div class="col s12"><br>
			  	<div class="card z-depth-5 col s12">
			  	 ...
			  	 <ul id="dropdown5" class="dropdown-content " style="height:40px;line-height:40px">
                      <li>
                      	<a href="{!! url('souscripteurs/preselectionne/listepdf') !!}" style="font-size:18px;">
                      ...
                      <li>
                      		<a href="{!!url('souscripteurs/preselectionne/listeexcel') !!}" style="font-size:18px">
			                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="20pt" height="20pt" viewBox="0 0 20 20" version="1.1">
									<g id="surface1">
				...
				<div class="row" >
					 <div class="col s12" style="margin-top:50px">
		    				<a class="dropdown-trigger  waves-light green btn-large" href="#!" data-target="dropdown5" style="margin-right:100px;margin-bottom:20px">Exporter en <i class="material-icons right">arrow_drop_down</i></a>
				  			<a style="width:350px;margin-bottom:20px"class="dropdown-trigger  waves-light green btn-large" href="#!" data-target="dropdown6" >Liste des souscripteurs  <i class="material-icons right">arrow_drop_down</i></a>
		  			</div>
		  		</div>
				
				<h4>Liste des souscripteurs 1ère catégorie</h4>
				  ...
				  <table class="responsive-table" id="table">
		 			...
		 			@foreach($souscripteurs as $souscripteur)
		 			 		<tr>
								...
								<td>
									<div class="row">
								      	<a class="waves-effect waves-light btn-small" title="Formé" href="{!! url('selection',$souscripteur->id) !!}"><i class="material-icons center">check_circle</i></a>
					  				</div>
					  			</td>
							</tr>
							@endforeach
...
@section('scripts')
    ...
    function tableUpdatePNm(nom_Id,prenom_Id) {
        $.get('{{ url('formation/prenom') }}/'+nom_Id+'/'+prenom_Id, function(data) {
        		$('#table tbody').empty();
        		$('#page').hide();
            $.each(data, function(index, souscripteurs) {
				...
				$("#table tbody").append("<tr>"+
											...
											"<td>"+souscripteurs.niveauEtude + "</td>"+
											"<td>"+ souscripteurs.created_at+ "</td>"+
											'<td><a class="selectionne waves-effect waves-light btn-small modal-trigger" title="Formé" href="selection/'+souscripteurs.id+'"><i class="material-icons center">check_circle</i></a></td>'+
    ...
    function tableUpdateNm(nom_Id) {
        $.get('{{ url('formation/nom') }}/'+nom_Id, function(data) {
        		$('#table tbody').empty();
        		$('#page').hide();
            $.each(data, function(index, souscripteurs) {
